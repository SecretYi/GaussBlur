...
static void gaussianBlur(int *pix, int w, int h, int radius) {
    int wm = w - 1;
    int hm = h - 1;
    int wh = w * h;
...
Java_com_example_BlurUtils_gaussBlur(JNIEnv *env, jclass __unused class, jobject bmp, jint radius) {
    AndroidBitmapInfo info = {0};
    int *data = NULL;
    AndroidBitmap_getInfo(env, bmp, &info);
    AndroidBitmap_lockPixels(env, bmp, (void **) &data);
    gaussianBlur(data, info.width, info.height, radius);
